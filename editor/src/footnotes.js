...
(function (S) {
  ...
  function buildFootnoteReference(n) {
  ...
  function buildFootnoteEntry(n, text) {
    const frag = document.createDocumentFragment();

    const dt = document.createElement("dt");
    dt.textContent = "Footnote " + n;
    frag.appendChild(dt);
    ...
    inv1.textContent = "Return to footnote ";
    const inv2 = document.createElement("span");
    inv2.className = "wb-inv";
    inv2.textContent = " referrer";
    a.appendChild(inv1);
    a.appendChild(document.createTextNode(String(n)));
    a.appendChild(inv2);
    rtn.appendChild(a);
  ...
  // return the <dl>.
  function ensureFootnotesAside(root) {
    ...
    button.addEventListener("click", () => {
      ...
      if (sel && sel.rangeCount) {
        ...
        if (liveRoot.contains(range.commonAncestorContainer)) {
          ...
          const ref = buildFootnoteReference(n);
          ...
          sel.removeAllRanges();
          sel.addRange(after);
          ensureFootnotesAside(liveRoot).appendChild(buildFootnoteEntry(n, text));
          onChange();
